  ...
  <div>
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="k8sResource_K8sResource" />
            <ExportExcel  template-id="k8sResource_K8sResource" filterDeleted/>
            <ImportExcel  template-id="k8sResource_K8sResource" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column sortable align="left" label="资源名称" prop="name" width="120" />

            <el-table-column sortable align="left" label="命名空间" prop="namespace" width="120" />

            <el-table-column sortable align="left" label="资源类型" prop="type" width="120" />

            <el-table-column sortable align="left" label="资源状态" prop="status" width="120" />

            <el-table-column sortable align="left" label="最后修改" prop="lastModified" width="180">
   <template #default="scope">{{ formatDate(scope.row.lastModified) }}</template>
</el-table-column>
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateK8sResourceFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.labels }}
                    ...
                    <el-descriptions-item label="资源注解">
    {{ detailForm.annotations }}
</el-descriptions-item>
                    <el-descriptions-item label="资源状态">
    {{ detailForm.status }}
</el-descriptions-item>
                    <el-descriptions-item label="资源规格">
    {{ detailForm.spec }}
</el-descriptions-item>
...
const getTableData = async() => {
  const table = await getK8sResourceList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteK8sResourceByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateK8sResourceFunc = async(row) => {
    const res = await findK8sResource({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteK8sResourceFunc = async (row) => {
    const res = await deleteK8sResource({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
